...
vtkImageView2D::vtkImageView2D()
{
    ...
    this->AnnotationStyle      = AnnotationStyle2;
...
void vtkImageView2D::UpdateDisplayExtent()
{
    ...
    case vtkImageView2D::SLICE_ORIENTATION_XY:
        this->GetImageActor()->SetDisplayExtent(w_ext[0], w_ext[1], w_ext[2], w_ext[3], slice, slice);
        break;

    case vtkImageView2D::SLICE_ORIENTATION_XZ:
        this->GetImageActor()->SetDisplayExtent(w_ext[0], w_ext[1], slice, slice, w_ext[4], w_ext[5]);
        break;

    case vtkImageView2D::SLICE_ORIENTATION_YZ:
    default:
        this->GetImageActor()->SetDisplayExtent(slice, slice, w_ext[2], w_ext[3], w_ext[4], w_ext[5]);
        break;
    }
    ...
    if (this->Renderer)
    {
        ...
        if (cam)
        {
            ...
            cam->SetClippingRange(distance - 3 * avg_spacing, distance + 3 * avg_spacing);
        }
    }
}
...
void vtkImageView2D::SetOrientationMatrix (vtkMatrix4x4* matrix)
{
    ...
    this->GetImageActor()->SetUserMatrix (this->OrientationMatrix);
    this->UpdateOrientation();
...
{
    this->GetImageActor()->SetInterpolate (val);
    this->Modified();
}
...
{
    return this->GetImageActor()->GetInterpolate();
}
...
vtkActor* vtkImageView2D::AddDataSet(vtkPointSet* arg, vtkProperty* prop)
{
    ...
    else
    {
        ...
        newprop->LightingOff();
        widget->GetActor()->SetProperty (newprop);
        newprop->Delete();
    }

...
medVTKSimpleViewItkDataImageInteractor::medVTKSimpleViewItkDataImageInteractor(medAbstractView *parent):
    medAbstractImageViewInteractor(parent), d(new medVTKSimpleViewItkDataImageInteractorPrivate)
{
    d->view = dynamic_cast<medAbstractImageView*>(parent);
    medVTKSimpleViewBackend* backend = static_cast<medVTKSimpleViewBackend*>(parent->backend());
...
medVTKSimpleViewItkDataImageInteractor::~medVTKSimpleViewItkDataImageInteractor()
{
    delete d;
    d = NULL;
}

QStringList medVTKSimpleViewItkDataImageInteractor::dataHandled()
{
...
QStringList medVTKSimpleViewItkDataImageInteractor::handled() const
{
    return medVTKSimpleViewItkDataImageInteractor::dataHandled();
}

QString medVTKSimpleViewItkDataImageInteractor::description() const
{
    return tr("Interactor for ITK images");
}


QString medVTKSimpleViewItkDataImageInteractor::identifier() const
{
    return "medVTKSimpleViewItkDataImageInteractor";
}

bool medVTKSimpleViewItkDataImageInteractor::registered()
{
    medViewFactory *factory = medViewFactory::instance();
    return factory->registerInteractor<medVTKSimpleViewItkDataImageInteractor>("medVTKSimpleViewItkDataImageInteractor",
                                                                  QStringList () << "medVtkView" <<
                                                                  medVTKSimpleViewItkDataImageInteractor::dataHandled());
}

QList<medAbstractParameterL*> medVTKSimpleViewItkDataImageInteractor::linkableParameters()
{
...
QList<medBoolParameterL*> medVTKSimpleViewItkDataImageInteractor::mouseInteractionParameters()
{
...
void medVTKSimpleViewItkDataImageInteractor::setInputData(medAbstractData *data)
{
    ...
    d->view2d->GetImageActor()->GetProperty()->SetInterpolationTypeToCubic();
...
void medVTKSimpleViewItkDataImageInteractor::removeData()
{
//    d->view2d->RemoveLayer(d->view->layer(d->imageData));
//    d->view3d->RemoveLayer(d->view->layer(d->imageData));
    qDebug() << "No layering yet";
}

template <typename IMAGE>
bool medVTKSimpleViewItkDataImageInteractor::SetViewInput(const char* type, medAbstractData* data, int layer)
{
    ...
    IMAGE* image = dynamic_cast<IMAGE*>((itk::Object*)(data->data()));

    if (!image)
        return false;

    vtkSmartPointer <vtkMetaImageData> metaImage = vtkMetaImageData::New();
    metaImage->SetItkImage(image);

    // Layer ignored here, should be added
    d->view2d->SetInput (metaImage->GetImageData(), metaImage->GetOrientationMatrix());
    d->view3d->SetInput (metaImage->GetImageData(), metaImage->GetOrientationMatrix());

    return true;
}

void medVTKSimpleViewItkDataImageInteractor::initParameters(medAbstractImageData* data)
{
    d->imageData = data;
...
void medVTKSimpleViewItkDataImageInteractor::initWindowLevelParameters(double *range)
{
...
void medVTKSimpleViewItkDataImageInteractor::setOpacity(double opacity)
{
...
void medVTKSimpleViewItkDataImageInteractor::setVisibility(bool visible)
{
//    if(visible)
//    {
...
QString medVTKSimpleViewItkDataImageInteractor::lut() const
{
    return d->lutParam->value();
}

void medVTKSimpleViewItkDataImageInteractor::setLut(QString value)
{
    ...
    if (d->view->layer(d->imageData) == 0)
        ...
        d->view2d->SetTransferFunctions(rgb, alpha);
    else
    {
    ...
    d->view3d->SetLookupTable(vtkLookupTableManager::GetLookupTable(value.toStdString()));
...
void medVTKSimpleViewItkDataImageInteractor::setPreset(QString preset)
{
...
QString medVTKSimpleViewItkDataImageInteractor::preset() const
{
  return d->presetParam->value();
}


QWidget* medVTKSimpleViewItkDataImageInteractor::buildToolBarWidget()
{
    d->slicingParameter->getSlider()->setOrientation(Qt::Horizontal);
    return d->slicingParameter->getSlider();
}

QWidget* medVTKSimpleViewItkDataImageInteractor::buildToolBoxWidget()
{
...
QWidget* medVTKSimpleViewItkDataImageInteractor::buildLayerWidget()
{
        this->opacityParameter()->getSlider()->setOrientation(Qt::Horizontal);
        return this->opacityParameter()->getSlider();
}

void medVTKSimpleViewItkDataImageInteractor::setWindowLevelFromMinMax()
{
...
//        d->view3d->SetColorLevel(level, d->view->layer(d->imageData));
    if(d->view2d->GetColorWindow() != window)
        d->view2d->SetColorWindow(window);
    if(d->view3d->GetColorWindow() != window)
        d->view3d->SetColorWindow(window);
    if(d->view2d->GetColorLevel() != level)
        d->view2d->SetColorLevel(level);
    if(d->view3d->GetColorLevel() != level)
        d->view3d->SetColorLevel(level);

    this->windowLevelParameter()->blockSignals(false);
}

void medVTKSimpleViewItkDataImageInteractor::setWindowLevel(QHash<QString,QVariant> values)
{
    if(values.size() != 2 )
    {
...
void medVTKSimpleViewItkDataImageInteractor::moveToSlice(int slice)
{
...
void medVTKSimpleViewItkDataImageInteractor::update()
{
    if(d->view->is2D())
        d->view2d->Render();
    else
...
void medVTKSimpleViewItkDataImageInteractor::updateWidgets()
{
    if(!d->view->is2D())
...
void medVTKSimpleViewItkDataImageInteractor::setUpViewForThumbnail()
{
    d->view2d->Reset();
...
void medVTKSimpleViewItkDataImageInteractor::updateImageViewInternalLayer()
{
...
void medVTKSimpleViewItkDataImageInteractor::updateSlicingParam()
{
    if(!d->view->is2D())
        return;
...
void medVTKSimpleViewItkDataImageInteractor::enableWIndowLevel(bool enable)
{
    if(enable)

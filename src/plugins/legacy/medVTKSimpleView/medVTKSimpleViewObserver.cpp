...
medVTKSimpleViewObserver::medVTKSimpleViewObserver()
{
    this->m_locked = false;
}
...
void medVTKSimpleViewObserver::setView (medVTKSimpleView *view)
{
    this->m_view = view;

    medVTKSimpleViewBackend* backend = static_cast<medVTKSimpleViewBackend*>(view->backend());
    this->view2d = backend->view2D;
    this->view3d = backend->view3D;
}

void medVTKSimpleViewObserver::Execute(vtkObject *caller, unsigned long event, void *callData)
{
    if(this->m_locked)
        return;
    ...
    // SIMPLE : no layer support, should be ok if we care that no second layer is ever added
    ...
    {
        // SIMPLE : Removed layer support (layer) in getcolorlevel and GetColorWindow

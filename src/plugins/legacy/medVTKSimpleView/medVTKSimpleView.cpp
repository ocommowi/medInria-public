...
#include <medVTKSimpleViewObserver.h>
...
class MEDVTKSIMPLEVIEWPLUGIN_EXPORT medVTKSimpleViewQVtkInteractorAdapter : public QVTKInteractorAdapter
{

public:

    medVTKSimpleViewQVtkInteractorAdapter(QObject* parentObject):
...
class MEDVTKSIMPLEVIEWPLUGIN_EXPORT medVTKSimpleViewQVtkGraphicsItem : public QVTKGraphicsItem
{
public:

    medVTKSimpleViewQVtkInteractorAdapter* mVtkViewIrenAdapter;
    QGLWidget *mGlWidget;
    int plic, ploc;

    medVTKSimpleViewQVtkGraphicsItem(QGLContext* ctx, QGraphicsItem* p = 0, QGLWidget* glWidget = 0):
        QVTKGraphicsItem(ctx, p), mGlWidget(glWidget), plic(0), ploc(0)
    {
        this->moveToThread(QApplication::instance()->thread());
        this->setSizePolicy(QSizePolicy::Ignored, QSizePolicy::Ignored);
        mVtkViewIrenAdapter = new medVTKSimpleViewQVtkInteractorAdapter(this);
    }
...
public:
    ...
    medVTKSimpleViewQVtkGraphicsItem *vtkItem;
    medVTKSimpleViewQGraphicsView *viewWidget;

    medVTKSimpleViewObserver *observer;

    medBoolParameterL *rubberBandZoomParameter;

    QScopedPointer<medVTKSimpleViewBackend> backend;
};

medVTKSimpleView::medVTKSimpleView(QObject* parent): medAbstractImageView(parent),
    d(new medVTKSimpleViewPrivate)
{
    ...
    d->viewWidget = new medVTKSimpleViewQGraphicsView;
    ...
    d->vtkItem = new medVTKSimpleViewQVtkGraphicsItem(const_cast<QGLContext*>(glWidget->context()), 0, glWidget);
    ...
    d->backend.reset(new medVTKSimpleViewBackend(d->view2d, d->view3d, d->renWin));

    d->observer = medVTKSimpleViewObserver::New();
...
medVTKSimpleView::~medVTKSimpleView()
{
...
QString medVTKSimpleView::s_identifier()
{
    return "medVTKSimpleView";
}

QString medVTKSimpleView::identifier() const
{
    return medVTKSimpleView::s_identifier();
}

bool medVTKSimpleView::registered()
{
    return medViewFactory::instance()->registerView<medVTKSimpleView>(QString("medVTKSimpleView"), QStringList()<<"medAbstractData");
}

medViewBackend * medVTKSimpleView::backend() const
{
    return d->backend.data();
}

QString medVTKSimpleView::description() const
{
    return "medVTKSimpleView";
}

QWidget* medVTKSimpleView::viewWidget()
{
    return d->viewWidget;
}

void medVTKSimpleView::reset()
{
    d->view2d->Reset();
    d->view3d->Reset();
    this->render();
...
QPointF medVTKSimpleView::mapWorldToDisplayCoordinates(const QVector3D & worldVec)
{
...
QVector3D medVTKSimpleView::mapDisplayToWorldCoordinates(const QPointF & scenePoint)
{
    // The following code is implemented without calling ren->SetWorldPoint,
    // because that generates an unnecessary modified event.
...
QVector3D medVTKSimpleView::viewCenter()
{
    vtkRenderer * ren;
    if(this->is2D())
...
QVector3D medVTKSimpleView::viewPlaneNormal()
{
    double vpn[3];
    vtkRenderer * ren;
    if(this->is2D())
...
QVector3D medVTKSimpleView::viewUp()
{
    double vup[3];
    vtkRenderer * ren;
    if(this->is2D())
...
qreal medVTKSimpleView::sliceThickness()
{
    double cr[2] = {0, 0};
...
void medVTKSimpleView::changeCurrentLayer()
{
    displayDataInfo(this->currentLayer());
}

void medVTKSimpleView::displayDataInfo(uint layer)
{
...
QImage medVTKSimpleView::buildThumbnail(const QSize &size)
{
...
void medVTKSimpleView::buildMouseInteractionParamPool(uint layer)
{
...
void medVTKSimpleView::saveMouseInteractionSettings(bool parameterEnabled)
{
    if(parameterEnabled)
    {
...
void medVTKSimpleView::enableRubberBandZoom(bool enable)
{
...
medBoolParameterL* medVTKSimpleView::rubberBandZoomParameter() const
{
    return d->rubberBandZoomParameter;
}

void medVTKSimpleView::setOffscreenRendering(bool isOffscreen)
{
    d->renWin->SetOffScreenRendering(isOffscreen ? 1 : 0);
}

void medVTKSimpleView::resetKeyboardInteractionModifier()
{
